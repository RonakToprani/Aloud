...
export function formatListened(totalSeconds: number): { figure: string; unit: string } {
  if (totalSeconds < 3600) {
    ...
    return { figure: integer.format(Math.floor(minutes)), unit: minutes < 1.5 && minutes >= 1 ? "minute" : "minutes" };
  }
    ...
    <p className={styles.strip}>
      ...
      {stats.listeningNow > 0 && (
        <>
          <span className={styles.stripDot}>·</span>
          <span className={styles.presenceDot} data-live="true" aria-hidden="true" />
          {integer.format(stats.listeningNow)} listening now
        </>
      )}
      {stats.listeningNow === 0 && stats.activeReaders > 0 && (
        <>
          <span className={styles.stripDot}>·</span>
          {integer.format(stats.activeReaders)} active this week
        </>
      )}
    </p>
  );
